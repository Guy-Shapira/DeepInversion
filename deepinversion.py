...
def get_images(net, bs=256, epochs=1000, idx=-1, var_scale=0.00005,
    ...
    for epoch in range(epochs):
        ...
        if competitive_scale != 0.0 and False:
    ...
    counter = 0
    for image in best_inputs.clone():
        counter += 1
        vutils.save_image(image,
                      './{}/output_{}_{}.png'.format(name_use, next_batch, counter),
...
def test():
    _, testloader = get_split_cifar100(64, 0, 50)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--teacher_weights', default="p1_192.pt", type=str, help='path to load weights of the model')
    ...
    prefix = args.prefix
    prefix += "/batch_" + str(counter) + "/"
    if not os.path.exists("./" + prefix):
        os.mkdir("./" + prefix)
    if not os.path.exists("./" + prefix + "/best_images/"):
        os.makedirs("./" + prefix + "/best_images/")
